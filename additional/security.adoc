...
[IMPORTANT]
I am thinking about adding a section on the Auth0 token used with the web UI. This is another security layer, because to generate the API token they need to be able to sign in to the UI. ANy opions?
...
[IMPORTANT]
I would like to put the above list into a table with a description for each, but couldn't find where these roles are defined.
