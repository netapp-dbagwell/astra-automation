...
|Accept
|Request
|If the value is `"*/*` or is not provided, `application/json` is returned in Content-Type response header. If the value is set to the Astra resource mimetype, the same mimetype is returned in the Content-Type header.
