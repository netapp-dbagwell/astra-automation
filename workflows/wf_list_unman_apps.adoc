...
[NOTE]
This workflow lists all of the applications. You can look at the output to determine which apps are unmanaged by examining the `managedState` field.
...
[source,curl]
curl --location --request GET 'https://astra.netapp.io/accounts/<ACCOUNT_ID>/topology/v1/apps?include=name,id,managedState' --header 'Content-Type: application/astra-app+json' --header 'Accept: application/astra-app+json' --header 'Authorization: Bearer <API_TOKEN>'
